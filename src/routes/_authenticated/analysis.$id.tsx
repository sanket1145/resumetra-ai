...
function AnalysisDetail() {
  ...
  const analysis = data;
  const score = Number(analysis.match_score);
  const matching = (analysis.matching_skills ?? []) as unknown as MatchedSkill[];
  const missing = (analysis.missing_skills ?? []) as unknown as MatchedSkill[];
  const extra = (analysis.extra_skills ?? []) as unknown as Array<{ name: string; category: string }>;
  const strengths = (analysis.strengths ?? []) as unknown as string[];
  const suggestions = (analysis.suggestions ?? []) as unknown as string[];
  const breakdown = analysis.score_breakdown as unknown as ScoreBreakdown | null;
  const jobTitle = analysis.job_descriptions?.title ?? "Untitled role";
  const company = analysis.job_descriptions?.company ?? null;
  ...
  function downloadReport() {
    const lines = [
      ...
      `Resume: ${analysis.resumes?.file_name ?? "Resume"}`,
      `Candidate: ${analysis.resumes?.candidate_name ?? "Not detected"}`,
      `Email: ${analysis.resumes?.candidate_email ?? "Not detected"}`,
      `Phone: ${analysis.resumes?.candidate_phone ?? "Not detected"}`,
      ...
      "",
      `Areas for improvement (${analysis.suggestion_source === "llm" ? "AI generated" : "rule based"}):`,
      ...suggestions.map((item) => `  - ${item}`),
      "",
      analysis.llm_summary ? `Summary: ${analysis.llm_summary}` : "",
    ].filter((line) => line !== "");
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-10">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {company ? `${company} · ` : ""}
            {analysis.resumes?.file_name ?? "Resume"} · {new Date(analysis.created_at).toLocaleString()}
          </p>
        </div>
      ...
      <section className="panel mt-6 p-6">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          {analysis.suggestion_source === "llm"
            ? "Generated by the language model from your extracted resume data."
            : "Generated from the rule-based analysis engine."}
        </p>
        {analysis.llm_summary ? (
          <p className="mt-4 rounded-lg bg-surface p-4 text-sm leading-relaxed">{analysis.llm_summary}</p>
        ) : null}
